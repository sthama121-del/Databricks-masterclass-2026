...
All three notebooks share one small, realistic **retail `orders` dataset** (18 rows), created entirely in-memory inside each notebook — no external files, no cloud storage, no Unity Catalog permissions required.
...
**Part 4 — SQL ↔ PySpark Joins & Multi-Table Data**
